...
# btn_reset = tk.Button(
#     frame_3_child_2, relief="raised", bd=0, text="Reset", bg=COLOR_BG_3, fg="#000")
...
def choose_filter():
    ...
    if radio_var.get() == 3:
        ...
        if(img is None): messagebox.showerror('Error','Chưa Tải ảnh lên!')
            ...
            menu = tk.Menu(root, tearoff=0)
            menu.add_command(label='OpenCv', command=lambda: sliderBar_action(lam_min_opencv2_active))
            menu.add_command(label='Without OpenCv', command=lambda: action_02())
